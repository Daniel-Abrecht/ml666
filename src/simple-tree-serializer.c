...
#define SERIALIZER_STATE \
  ...
  X(SERIALIZER_W_CONTENT_TEXT) \
  X(SERIALIZER_W_CONTENT_BASE64) \
    ...
    while(i<buf.length && j<outbuf.length-4){
      ...
      if(ch > ' '){
        ...
        switch(ch){
          case '*': case '/': case '>':
          case '`': case '=': case ' ':
          case '\\'  : outbuf.data[j++] = ch; break;
          case '\a'  : outbuf.data[j++] = 'a'; break;
          case '\b'  : outbuf.data[j++] = 'b'; break;
          case '\x1B': outbuf.data[j++] = 'e'; break;
          case '\f'  : outbuf.data[j++] = 'f'; break;
          case '\n'  : outbuf.data[j++] = 'n'; break;
          case '\t'  : outbuf.data[j++] = 't'; break;
          case '\v'  : outbuf.data[j++] = 'v'; break;
          default: {
            outbuf.data[j++] = 'x';
            outbuf.data[j++] = ch >> 4;
